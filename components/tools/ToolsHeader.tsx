...
export function ToolsHeader() {
  ...
  return (
    <header className="sticky top-0 z-30 border-b border-neutral-200/80 bg-white/80 backdrop-blur-xl dark:border-white/10 dark:bg-neutral-950/80">
      <div className="mx-auto flex max-w-6xl items-center justify-between px-6 py-3.5">
            ...
            {TOOL_CATEGORIES.map((category) => (
              ...
              >
                ...
                {openCategory === category.name && (
                  <div className="absolute left-0 top-full w-64 rounded-2xl border border-neutral-200 bg-white p-2 shadow-xl dark:border-white/10 dark:bg-neutral-900">
                    {category.tools.map((tool) => {
                      ...
                      return isLive ? (
                        ...
                        <div
                          key={tool.slug}
                          className="flex items-center justify-between rounded-xl px-3 py-2 text-sm text-neutral-400 dark:text-neutral-600"
                        >
                          {tool.name}
        ...
        <div className="flex items-center gap-3">
          ...
          <Link
            href="/"
            className="hidden rounded-lg border border-neutral-300 px-3 py-1.5 text-sm font-medium text-neutral-600 transition hover:bg-neutral-100 dark:border-white/15 dark:text-neutral-300 dark:hover:bg-white/5 sm:block"
          >
            InstaCard Home
          </Link>
          <button
